...
export interface PostgresStorageConfig {
  ...
  tablePrefix?: string; // Default: 'notif_'
}
...
export class PostgresStorageAdapter implements StorageAdapter {
  ...
  async findByUser(userId: string, filters?: NotificationFilters): Promise<Notification[]> {
    ...
    return result.rows.map((row: any) => this.rowToNotification(row));
  }
  ...
  async getNotificationsByGroup(groupId: string): Promise<Notification[]> {
    const query = `
      SELECT * FROM ${this.tablePrefix}notifications
      WHERE group_id = $1
  ...
  async getScheduledNotifications(before?: Date): Promise<Notification[]> {
    const query = `
      SELECT * FROM ${this.tablePrefix}notifications
